...
import { Heart, LogOut, Users, Calendar, AlertCircle, TrendingUp } from 'lucide-react'
...
function DoctorDashboard({ onLogout, currentUser }) {
  ...
  const stats = [
    { label: 'Patients Today', value: 8, icon: Users, color: 'bg-blue-100', textColor: 'text-primary' },
    { label: 'Consultations', value: 45, icon: Calendar, color: 'bg-green-100', textColor: 'text-success' },
    { label: 'Pending Cases', value: 3, icon: AlertCircle, color: 'bg-warning bg-opacity-20', textColor: 'text-warning' },
    { label: 'Patient Rating', value: '4.8', icon: TrendingUp, color: 'bg-purple-100', textColor: 'text-purple-600' }
  ]

  const todayAppointments = [
    { id: 1, patient: 'John Doe', time: '9:00 AM', type: 'Video', status: 'Completed' },
    { id: 2, patient: 'Sarah Smith', time: '10:30 AM', type: 'In-Person', status: 'In Progress' },
    { id: 3, patient: 'Mike Johnson', time: '11:30 AM', type: 'Video', status: 'Upcoming' },
    { id: 4, patient: 'Lisa Brown', time: '2:00 PM', type: 'In-Person', status: 'Upcoming' }
  ]

  const urgentCases = [
    { id: 1, patient: 'Alex Wilson', issue: 'High fever (102°F)', urgency: 'High', time: '15 minutes ago' },
    { id: 2, patient: 'Emma Davis', issue: 'Chest discomfort', urgency: 'Critical', time: '5 minutes ago' }
  ]
  ...
  return (
    <div className="min-h-screen bg-light-gray">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-dark-gray">{currentUser?.name || 'Doctor'}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-4xl font-bold text-dark-gray mb-8">Doctor Dashboard 👨‍⚕️</h1>
              ...
              {urgentCases.map((case_) => (
                <div key={case_.id} className="p-4 bg-red-50 rounded-lg border border-red-200">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-semibold text-dark-gray">{case_.patient}</h3>
                    <span className={`badge-${case_.urgency === 'Critical' ? 'danger' : 'warning'}`}>
                      {case_.urgency}
                    </span>
                  </div>
        ...
        <div className="card">
          <h2 className="text-xl font-bold text-dark-gray mb-4">Today's Appointments</h2>
            ...
            <table className="w-full">
              ...
              <tbody>
                {todayAppointments.map((apt) => (
                  <tr key={apt.id} className="border-b border-border-gray hover:bg-light-gray">

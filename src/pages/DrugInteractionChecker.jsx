...
function DrugInteractionChecker({ onLogout, currentUser }) {
  ...
  const filteredDrugs = mockDrugDatabase.filter(
    (drug) =>
  ...
  const addDrug = (drugName) => {
    setSelectedDrugs([
      ...selectedDrugs,
      { name: drugName, dose: "1000mg", frequency: "Daily" },
    ]);
    setSearchInput("");
  };
  ...
  const interactions = [
    {
      ...
      severity: "critical",
      description:
        "Combination increases risk of GI bleeding and stomach ulcers",
    },
    {
      drugs: ["Lisinopril", "Ibuprofen"],
      severity: "major",
      description:
        "May reduce effectiveness of blood pressure control and affect kidney function",
    },
    {
              ...
              <div className="mb-4">
                ...
                <div className="space-y-3">
                  {selectedDrugs.map((drug, i) => (
                    <div
                      key={i}
                      className="p-3 bg-light-gray rounded-lg flex justify-between items-start"
                    >
            ...
            {interactions.length > 0 ? (
              <div className="space-y-4">
                {interactions.map((interaction, i) => (
                  <div
                    key={i}
                    className={`p-6 rounded-lg border ${getSeverityColor(interaction.severity)}`}
                  >
